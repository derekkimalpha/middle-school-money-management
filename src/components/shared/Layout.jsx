...
const NavItem = ({ item, isActive, onNavigate }) => (
  <button
    ...
    className={`
      w-full text-left px-3 py-2 rounded-ds-md text-[14px] font-ds-sans font-medium transition-all
      ${isActive
        ? 'bg-ds-primary text-ds-canvas'
        : 'text-ds-secondary hover:text-ds-primary hover:bg-ds-overlay'
      }
    `}
  >
...
export const Layout = ({
  ...
  return (
    <div className="flex h-screen bg-ds-canvas text-ds-primary font-ds-sans transition-colors duration-200">
      ...
      <aside
        className={`
          flex flex-col select-none overflow-hidden bg-ds-canvas
          md:w-[248px] md:relative md:translate-x-0 md:z-auto md:border-r md:border-ds-hairline
          ${isMobile
            ? 'fixed inset-y-0 left-0 z-50 w-[280px] border-r border-ds-hairline transform transition-transform duration-300'
            : ''
          }
      ...
      >
        ...
        <div className="px-5 pt-7 pb-5">
          ...
          >
            <div className="w-9 h-[26px] flex items-center justify-center text-ds-primary">
              <AlphaBird className="w-full h-full" />
            </div>
            <div className="min-w-0 text-left">
              <div className="text-[15px] font-semibold text-ds-primary tracking-tight leading-none">My Money</div>
              <div className="text-[11px] text-ds-tertiary font-medium mt-1 truncate">Alpha School</div>
            </div>
          </button>
        </div>
        ...
        <nav className="flex-1 px-3 pt-2 space-y-0.5 overflow-y-auto">
          <div className="px-3 pt-2 pb-1.5">
            <p className="text-ds-tertiary text-[10px] tracking-[0.08em] uppercase font-semibold">Main</p>
          </div>
          {navItems.slice(0, Math.ceil(navItems.length / 2)).map((item) => (
            <NavItem key={item.id} item={item} isActive={activePage === item.id} onNavigate={handleNavClick} />
          ))}

          {navItems.length > 1 && (
            <>
              <div className="px-3 pt-4 pb-1.5">
                <p className="text-ds-tertiary text-[10px] tracking-[0.08em] uppercase font-semibold">Account</p>
              </div>
        ...
        <div className="px-3 pb-4 pt-3 space-y-2 border-t border-ds-hairline mt-auto">
          {/* Profile pill */}
          <div className="flex items-center gap-2.5 px-2.5 py-2 rounded-ds-lg border border-ds-hairline">
            <div className="w-8 h-8 rounded-full bg-ds-accent-soft text-ds-accent flex items-center justify-center font-semibold text-[12px] flex-shrink-0">
              {initials}
            </div>
            <div className="min-w-0 flex-1">
              <div className="text-[13px] font-semibold text-ds-primary truncate">{firstName || user?.name}</div>
              {role && <div className="text-[10px] text-ds-tertiary font-medium capitalize">{role}</div>}
            </div>
          </div>

          {/* Mode toggle + sign out (two-up row) */}
          <div className="flex items-center gap-1.5">
            <button
              onClick={toggleTheme}
              className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-ds-md text-[12px] font-medium text-ds-secondary hover:text-ds-primary hover:bg-ds-overlay transition-all"
            ...
            <button
              onClick={onSignOut}
              className="flex items-center justify-center w-9 h-9 rounded-ds-md text-ds-secondary hover:text-ds-negative hover:bg-ds-negative-soft transition-all"
              aria-label="Sign out"
              title="Sign out"
            >
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        {/* Topbar — minimal, hairline border */}
        <header className="sticky top-0 z-20 bg-ds-canvas/95 backdrop-blur-md border-b border-ds-hairline px-6 md:px-8 h-[56px] flex items-center justify-between">
          <div className="flex items-center gap-3">
            {isMobile && (
              <button
                onClick={() => setSidebarOpen(!sidebarOpen)}
                className="md:hidden flex items-center justify-center w-9 h-9 rounded-ds-md text-ds-primary hover:bg-ds-overlay transition-colors"
            ...
            <h1 className="text-[15px] font-semibold text-ds-primary tracking-tight">
              {navItems.find((item) => item.id === activePage)?.label || 'My Money'}
            </h1>
          </div>
          {user && (
            <div className="hidden sm:flex items-center gap-2 text-[12px] text-ds-tertiary font-medium">
